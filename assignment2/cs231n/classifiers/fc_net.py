...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache2 = affine_forward(relu1, self.params['W2'], self.params['b2'])
        ...
        ############################################################################
        # pass
        # softmax_scores = scores #softmax(scores)
        ...
        dx, grads['W2'], grads['b2'] = affine_backward(d_softmax, cache2)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        self.params['W'+str(len(hidden_dims)+1)] = np.random.normal(loc=0, scale=weight_scale, size=(hidden_dims[-1], num_classes))
    ...
    def loss(self, X, y=None):
        ...
        def compute_layer_forward(layer_n, out, cache_dict):
            # params, cache_dict = params, cache_dict
            out, cache = affine_forward(out, self.params['W'+str(layer_n)], self.params['b'+str(layer_n)])
            cache_dict['aff_'+str(layer_n)+'_cache'] = cache
            if self.use_batchnorm:
                out, cache = batchnorm_forward(out, self.params['gamma'+str(layer_n)], \
                    self.params['beta'+str(layer_n)], self.bn_params[layer_n-1])
        ...
        for i in range(len(self.hidden_dims)):
            out, cache_dict = compute_layer_forward(i+1, out, cache_dict)
        
        last = str(len(self.hidden_dims)+1)
        scores, cache = affine_forward(out, self.params['W'+last], self.params['b'+last])
        cache_dict['aff_'+last+'_cache'] = cache
        ...
        # pass
        loss, d_softmax = softmax_loss(scores, y)
        for i in range(1, len(self.hidden_dims)+2):
            loss += 0.5 * self.reg * np.sum(self.params['W'+str(i)] * self.params['W'+str(i)])

        dx, grads['W'+last], grads['b'+last] = affine_backward(d_softmax, cache_dict['aff_'+last+'_cache'])  
        grads['W'+last] += self.reg * self.params['W'+last]
        ...
        for i in range(len(self.hidden_dims), 0, -1):
            ...
            dx, grads['W'+str(i)], grads['b'+str(i)] = affine_backward(dx, cache_dict['aff_'+str(i)+'_cache'])
